...
public class UI {
	...
	public static ChessPosition readChessPosition(Scanner scan) {
		try {
			...
			throw new InputMismatchException("Erro ao informar posição. Valores válidos entre a1 até h8.");
		}
	}
	...
	public static void printMatch(ChessMatch chessMatch, List<ChessPiece> capturedPieces) {
		...
		System.out.println(); 
		System.out.println("Turno atual: " + chessMatch.getTurn());
	...
	private static void printCapturedPieces(List<ChessPiece> capturedPieces) {
		List<ChessPiece> whiteCapturedPieces = capturedPieces.stream()
				.filter( pieces -> pieces.getColor() == Color.WHITE).collect(Collectors.toList());
		
		List<ChessPiece> blackCapturedPieces = capturedPieces.stream()
				.filter( pieces -> pieces.getColor() == Color.BLACK).collect(Collectors.toList());
